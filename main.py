...
#we need to sort the players list since the algorithm  will sort it too
#in this part the may have an O complexity of O(n.logn)
players = sorted(players, key = lambda i: i['h_in'])
#the algorithm  is going to find the index of the matching pairs
solution = findPairs.FindPairs(l,len(l),int(sys.argv[1])).findPairs()
if len(solution) > 0 :
    for i in solution:
        print(players[i[0]].get("first_name") + " " + players[i[0]].get("last_name")+"          "+players[i[1]].get("first_name")+" "+players[i[1]].get("last_name"))
else :
